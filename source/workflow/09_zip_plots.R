...
# plot poverty
ggplot() +
  geom_point(data = stl_city_zip_sf, mapping = aes(x = confirmed_rate, pvty_pct), color = "#515151", size = 2) +
  geom_point(data = stl_city_zip_sf2, mapping = aes(x = confirmed_rate, pvty_pct), color = "#1B9E77", size = 2) +
  geom_smooth(data = stl_city_zip_sf2, mapping = aes(x = confirmed_rate, pvty_pct), method = "lm", color = "#D95F02", size = 2) +
...
# plot race
ggplot() +
  geom_point(data = stl_city_zip_sf, mapping = aes(x = confirmed_rate, blk_pct), color = "#515151", size = 2) +
  geom_point(data = stl_city_zip_sf2, mapping = aes(x = confirmed_rate, blk_pct), color = "#1B9E77", size = 2) +
  geom_smooth(data = stl_city_zip_sf2, mapping = aes(x = confirmed_rate, blk_pct), method = "lm", color = "#D95F02", size = 2) +
  scale_x_continuous(limits = c(0,3), breaks = c(0,.5,1,1.5,2,2.5,3)) +
