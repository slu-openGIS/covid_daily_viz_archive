...
cols <- c("St. Louis City" = values$pal[1], "St. Louis" = values$pal[2], 
          "Kansas City" = values$pal[3], "Mississippi" = values$pal[4], 
          "Pemiscot" = values$pal[5], "Scott" = values$pal[6],  
          "Stoddard" = values$pal[7], "New Madrid" = values$pal[8], 
          "Butler" = values$pal[9], "Dunklin" = values$pal[10])

# define focal metros
county_focal <- c("29510", "29189", "29511", regional_geoids$semo)
...
county_subset %>%
  mutate(case_avg_rate = ifelse(geoid == 29133 & 
                                  (report_date == "2020-11-25" | report_date == "2020-11-29"), 160, case_avg_rate),
         case_avg_rate = ifelse(geoid == 29133 & 
                                  (report_date >= "2020-11-26" & report_date <= "2020-11-28"), NA, case_avg_rate)
  ) -> county_subset
...
p <- facet_rate(county_subset, 
                ...
                caption = paste0(values$caption_text_census,"\nValues above 160 for Mississippi County truncated to increase readability"))

# values$caption_text_census

## save plot
save_plots(filename = "results/high_res/county_semo/e_new_case.png", plot = p, preset = "lg")
save_plots(filename = "results/low_res/county_semo/e_new_case.png", plot = p, preset = "lg", dpi = 72)
...
## save plot
save_plots(filename = "results/high_res/county_semo/e_new_case_last21.png", plot = p, preset = "lg")
save_plots(filename = "results/low_res/county_semo/e_new_case_last21.png", plot = p, preset = "lg", dpi = 72)
