...
Missouri currently releases data on race and ethnicity based on the number of people who have initiated vaccinations. Initiated means individuals have received an initial dose of the Moderna or Pfizer vaccines, or the single dose of the Johnson & Johnson vaccine.

### Static Plot

```{r static-race, echo=FALSE, out.width = '100%'}
knitr::include_graphics("img/mo_individual/c_race_vaccine_total.png")
```

Download: [<a href="https://raw.githubusercontent.com/slu-openGIS/covid_daily_viz/master/results/high_res/mo_individual/c_race_vaccine_total.png" target="_blank">High-res</a>] [<a href="https://raw.githubusercontent.com/slu-openGIS/covid_daily_viz/master/results/low_res/mo_individual/c_race_vaccine_total.png" target="_blank">Low-res</a>]

### Data Table

```{r state-vax-table, echo=FALSE, out.width = '100%'}
race_vax %>%
  select(value, initiated, iniitiated_rate) %>%
  filter(value %in% c("Two or More") == FALSE) %>%
  arrange(desc(iniitiated_rate)) %>% 
  mutate(iniitiated_rate = round(iniitiated_rate, digits = 0)) %>%
  rename(
    Identity = value,
    `Cumulative Initiated Vaccinations` = initiated,
    `Rate per 1,000 Individuals` = iniitiated_rate
  ) -> data_table
