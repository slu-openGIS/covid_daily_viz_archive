...
***Please note that these data have been removed temporarily because of very high quantities of missing data. The infection demographic data have always had missing data, but it hovered around 16-18%. The amount is now more than 55%. This plot will be replaced once the issues with missing data are rectified.***
...
state_data %>%
  select(value, deaths_est, mortality_rate) %>% # cases_est, case_rate, 
  mutate(
    deaths_est = round(deaths_est, digits = 0),
    mortality_rate = round(mortality_rate, digits = 2)
  ) %>% # cases_est = round(cases_est, digits = 0), case_rate = round(case_rate, digits = 2),
  rename(
    Identity = value,
    `Estimated Deaths` = deaths_est,
    `Deaths Per Capita` = mortality_rate
  ) -> data_table # `Estimated Cases` = cases_est, `Cases Per Capita` = case_rate,
