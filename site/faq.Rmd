...
The website Datawrapper has a <a href="https://blog.datawrapper.de/weeklychart-logscale/" target="_blank">great explainer</a> on how to read log plots. The key takeway from their explaination is that "Log scales show relative values instead of absolute ones." Instead of showing absolute change, they show us the *rate* at which change happens. These are useful for comparing how COVID-19, for example, spreads between multiple counties. The steeper the line for a particular jurisdiction, the faster the rate of growth.
