...
zip_snapshot2 <- st_read(here::here("data", "MO_HEALTH_Covid_Tracking", "data", "zip", "daily_snapshot_regional.geojson"), 
                        stringsAsFactors = FALSE) 
...
regional_counties <- filter(regional_counties, GEOID %in% c("29071", "29099", 
                      "29113", "29183", "29189", "29219", "29510"))

# calculate breaks
bins <- map_bins(zip_snapshot2, var = "case_avg_rate", style = "quantile", classes = 5, dig_lab = 2,
                 round = 2)
bins[1] <- round_any(x = min(zip_snapshot2$case_avg_rate, na.rm = TRUE), accuracy = .01, f = floor)
bins[length(bins)] <- round_any(x = max(zip_snapshot2$case_avg_rate, na.rm = TRUE), accuracy = .01, f = ceiling)

# create color palette
pal <- colorBin("RdPu", domain = zip_snapshot2$case_avg_rate, bins = bins)
...
leaflet() %>%
  ...
  addPolygons(
    ...
    highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE),
    popup = paste0("<b>Zip:</b> ", zip_snapshot2$GEOID_ZCTA, " <br> ",
                 "<b>Current Cumulative Count:</b> ", zip_snapshot2$cases, "<br>",
                 "<b>Current 14-day Average Rate per 10,000:</b> ", round(zip_snapshot2$case_avg_rate, digits = 2))
  )  %>%
  addPolylines(
    data = regional_counties,
    color = "#000000",
